...
/**
 * @method mixed userID() 获取当前登录用户ID
 * @method mixed userOpenid() 获取当前登录用户Openid
 * @method mixed uuId() 获取当前登录用户UUID
 * @method mixed userInfo() 获取当前登录用户信息
 */
class Request extends \think\Request
{
        ...
        if (is_array($str)) {
            foreach ($str as &$v) {
                if (is_array($v)) {
                    foreach ($v as &$vv) {
                        if (!is_array($vv)) {
                            $vv = $this->replaceWord($vv);
                        }
                    }
                } else {
                    $v = $this->replaceWord($v);
                }
            }
        } else {
            $str = $this->replaceWord($str);
        }
        return $str;
    }
     ...
     */
    public function replaceWord($str)
    {
        $farr = [
