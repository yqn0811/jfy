...
use app\index\controller\remote;
...
class UploadService
{
    ...
    public function checkUpImagesExt($files, $file_type=1)
    {
        ...
        // 会员级别的单文件大小限制（仅对图片生效）
        // 非后台图库路径（/index/pic）下走用户上下文
        try{
            if($file_type == 1 && strpos(request()->url(), '/index/pic') === false){
                $uid = request()->userID();
                if(!empty($uid)){
    ...
    {
        if(strpos(request()->url(), '/index/pic') === false){
            $uid = request()->userID();
            $this->syncUserEntitlementsQuietly($uid);
            $user = WdXcxUser::where('id', $uid)->find();
            if(!$user){
            ...
            $limitSize = (new UploadFileRecordService($this->uniacid))->getSurplusRemoteSize();
        }
        if($limitSize != -1){
