...
//                    $this->bfivepage(),
    ...
    protected  function totalshow()
    {
        ...
        $totalhtml="<li class=\"disabled\"><span>共".$this->lastPage()."页</span></li>";
        return $totalhtml;
     ...
     * @return true
     */
    private function removeXssStr($str)
    {
//        $ra1 = Array('javascript', 'vbscript', 'expression', 'applet', 'meta', 'xml', 'blink', 'link', 'style', 'script', 'embed', 'object', 'iframe', 'frame', 'frameset', 'ilayer', 'layer', 'bgsound', 'title', 'base');
        $ra1 = Array('javascript', 'vbscript', 'expression', 'blink', 'script', 'embed', 'object', 'iframe', 'frame', 'frameset', 'ilayer', 'layer', 'bgsound', 'title', 'base');
    ...
    protected  function gopage()
    {
        ...
        if(empty($_REQUEST['page'])){
            $params .="<input class='layui-input' type='number' min='1' name='page'  value='1' max='{$this->lastPage()}'  placeholder='页码'>";
        }
//        if(!empty($_REQUEST['page'])){//分页进来带参数
        ...
        foreach($query_param as $k=>$v){
            $k = remove_xss($k);
            $v = remove_xss($v);
            if(!$this->removeXssStr($v)){
                if($k !== 's'){
                    if($k == 'page'){
                        $v = intval($v);
                        $params .="<input class='layui-input' type='number' min='1' max='{$this->lastPage()}'  name='page'  value='{$v}'  placeholder='页码'>";
                    }else{
