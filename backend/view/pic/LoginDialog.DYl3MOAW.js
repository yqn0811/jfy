import { defineComponent, ref, onMounted, watch, useSSRContext, mergeProps, withCtx, createTextVNode, createVNode, createBlock, createCommentVNode, openBlock } from "vue";
import { p as pcApi, e as DialogFooter, f as DialogDescription, D as DialogTitle, a as DialogHeader, b as DialogContent, c as Dialog, d as authStore } from "./DialogTrigger.BEhXrbNy.js";
import { B as Button } from "./index.DRLhNP3M.js";
...
const _sfc_main = defineComponent({ __name: "LoginDialog", props: { open: { type: Boolean } }, emits: ["update:open", "login-success"], setup(__props, { expose: __expose, emit: __emit }) {
  __expose();
  const props = __props, emit = __emit, isClient = ref(true), loginStatus = ref("loading"), qrCodeUrl = ref(""), scene = ref("");
  let pollTimer = null;
  const stopPolling = () => {
    ...
    loginStatus.value = "loading", stopPolling();
    try {
      ...
      loginStatus.value = "expired", toast.error(error?.message || "二维码生成失败");
    }
  };
  onMounted(() => {
  ...
  const __returned__ = { props, emit, isClient, loginStatus, qrCodeUrl, scene, get pollTimer() {
    return pollTimer;
  }, set pollTimer(v) {
    pollTimer = v;
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(ssrRenderComponent($setup.Dialog, mergeProps({ open: $props.open, "onUpdate:open": $setup.handleUpdateOpen }, _attrs), { default: withCtx((_, _push2, _parent2, _scopeId) => {
    if (_push2) _push2(ssrRenderComponent($setup.DialogContent, { class: "sm:max-w-[400px] gap-6" }, { default: withCtx((_2, _push3, _parent3, _scopeId2) => {
      ...
      }), _: 1 }, _parent3, _scopeId2)), _push3(`<div class="flex flex-col items-center justify-center space-y-4 py-4"${_scopeId2}><div class="relative w-48 h-48 border border-border rounded-lg bg-white p-2 flex items-center justify-center overflow-hidden"${_scopeId2}>`), $setup.loginStatus !== "success" && $setup.qrCodeUrl ? _push3(`<img${ssrRenderAttr("src", $setup.qrCodeUrl)} alt="WeChat Login QR Code" class="${ssrRenderClass([$setup.loginStatus === "expired" && "blur-[2px] opacity-20", "w-full h-full object-contain"])}"${_scopeId2}>`) : $setup.loginStatus === "loading" ? (_push3(`<div class="flex flex-col items-center text-muted-foreground"${_scopeId2}>`), _push3(ssrRenderComponent($setup.SafeIcon, { name: "Loader2", size: 30, class: "animate-spin mb-2" }, null, _parent3, _scopeId2)), _push3(`<p class="text-sm"${_scopeId2}>二维码生成中</p></div>`)) : _push3("<!---->"), $setup.loginStatus === "success" ? (_push3(`<div class="flex flex-col items-center animate-in fade-in zoom-in duration-300"${_scopeId2}><div class="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center mb-3"${_scopeId2}>`), _push3(ssrRenderComponent($setup.SafeIcon, { name: "Check", size: 32, class: "text-primary" }, null, _parent3, _scopeId2)), _push3(`</div><p class="text-primary font-medium"${_scopeId2}>登录成功</p></div>`)) : _push3("<!---->"), $setup.loginStatus === "expired" ? (_push3(`<div class="absolute inset-0 flex flex-col items-center justify-center bg-background/60 backdrop-blur-[1px]"${_scopeId2}><p class="text-sm font-medium mb-3"${_scopeId2}>二维码已失效</p>`), _push3(ssrRenderComponent($setup.Button, { size: "sm", variant: "primary", onClick: $setup.handleRefresh }, { default: withCtx((_3, _push4, _parent4, _scopeId3) => {
