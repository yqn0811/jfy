...
import { p as pcApi, e as DialogFooter, h as DialogDescription, D as DialogTitle, a as DialogHeader, b as DialogContent, c as Dialog } from "./DialogTrigger.CEjexOPD.js";
import { B as Button } from "./index.C9lvc2cP.js";
import { I as Input } from "./Input.BXzCSOrK.js";
import { S as SafeIcon } from "./SafeIcon.C8oSH_I5.js";
import { ssrRenderComponent, ssrRenderAttr } from "vue/server-renderer";
import { _ as _export_sfc } from "./BaseLayout.yWDmk07z.js";
const _sfc_main = defineComponent({ __name: "ShareDialog", props: { open: { type: Boolean }, productId: {}, targetUserId: {}, shareCode: {} }, emits: ["update:open"], setup(__props, { expose: __expose, emit: __emit }) {
  __expose();
  const props = __props, emit = __emit, shareUrl = ref(""), miniCodeUrl = ref(""), miniPath = ref(""), isLoadingShare = ref(false), buildShareUrl = () => {
    ...
    props.shareCode ? params.set("code", props.shareCode) : props.targetUserId && params.set("uid", props.targetUserId);
    const url = new URL("./product-detail.html", window.location.href);
    return url.search = params.toString(), url.toString();
  }, loadShareData = async () => {
    if (shareUrl.value = buildShareUrl(), miniCodeUrl.value = "", miniPath.value = "", !(!props.targetUserId && !props.shareCode || !props.productId)) {
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  _push(ssrRenderComponent($setup.Dialog, mergeProps({ open: $props.open, "onUpdate:open": ($event) => $setup.emit("update:open", $event) }, _attrs), { default: withCtx((_, _push2, _parent2, _scopeId) => {
    if (_push2) _push2(ssrRenderComponent($setup.DialogContent, { class: "max-w-[500px]" }, { default: withCtx((_2, _push3, _parent3, _scopeId2) => {
      ...
      }), _: 1 }, _parent3, _scopeId2)), _push3(`<div class="space-y-4 py-4"${_scopeId2}><div class="space-y-2"${_scopeId2}><label class="text-sm font-medium"${_scopeId2}>分享链接</label><div class="flex gap-2"${_scopeId2}>`), _push3(ssrRenderComponent($setup.Input, { "model-value": $setup.shareUrl, readonly: "", class: "flex-1 bg-muted/50 text-sm" }, null, _parent3, _scopeId2)), _push3(ssrRenderComponent($setup.Button, { variant: "outline", size: "sm", class: "px-3", onClick: $setup.handleCopyLink }, { default: withCtx((_3, _push4, _parent4, _scopeId3) => {
      ...
      }), _: 1 }, _parent3, _scopeId2)), _push3(`</div></div><div class="space-y-2 pt-2 border-t"${_scopeId2}><label class="text-sm font-medium"${_scopeId2}>二维码分享</label><div class="flex justify-center p-4 bg-muted/30 rounded-lg"${_scopeId2}>`), $setup.miniCodeUrl ? _push3(`<img${ssrRenderAttr("src", $setup.miniCodeUrl)} alt="产品小程序码" class="w-40 h-40 border border-border rounded object-contain bg-white"${_scopeId2}>`) : $setup.isLoadingShare ? _push3(ssrRenderComponent($setup.SafeIcon, { name: "Loader2", size: 28, class: "animate-spin text-muted-foreground" }, null, _parent3, _scopeId2)) : _push3(ssrRenderComponent($setup.SafeIcon, { name: "QrCode", size: 36, class: "text-muted-foreground" }, null, _parent3, _scopeId2)), _push3("</div></div></div>"), _push3(ssrRenderComponent($setup.DialogFooter, null, { default: withCtx((_3, _push4, _parent4, _scopeId3) => {
