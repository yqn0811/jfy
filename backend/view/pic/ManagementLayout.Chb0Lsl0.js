...
import { _ as _export_sfc, S as SafeIcon, $ as $$BaseLayout } from "./SafeIcon.D7kIP4uZ.js";
...
import { A as AvatarFallback, a as AvatarImage, b as Avatar, D as DropdownMenuTrigger, f as DropdownMenuSeparator, g as DropdownMenuLabel, c as DropdownMenuItem, d as DropdownMenuContent, e as DropdownMenu } from "./index.DlIl1dUz.js";
import { d as authStore, i as isLocalMockEnabled, p as pcApi } from "./DialogTrigger.puU1MCQr.js";
import { cva } from "class-variance-authority";
import { d as SheetContent, e as Sheet } from "./index.CFwxfxgk.js";
...
const _sfc_main = defineComponent({ __name: "AuthGate", setup(__props, { expose: __expose }) {
  __expose();
  const isReady = ref(false), isAuthenticated = ref(false), showLoginDialog = ref(false), enterMockSession = async () => {
    const data = await pcApi.checkLoginStatus("mock_scene"), token = data?.token || data?.access_token || data?.authorization;
    token && authStore.setToken(token), (data?.user || data?.user_info) && authStore.setUser(data.user || data.user_info);
  }, verifyLogin = async () => {
    if (authStore.consumeCallbackToken(), !authStore.isLoggedIn() && isLocalMockEnabled() && await enterMockSession(), isAuthenticated.value = authStore.isLoggedIn(), !isAuthenticated.value) {
  ...
  const __returned__ = { isReady, isAuthenticated, showLoginDialog, enterMockSession, verifyLogin, handleLoginSuccess, SafeIcon, LoginDialog };
  return Object.defineProperty(__returned__, "__isScriptSetup", { enumerable: false, value: true }), __returned__;
} });
