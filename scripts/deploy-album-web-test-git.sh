...
SERVER_SOURCE_DIR="${SERVER_SOURCE_DIR:-/opt/jfy/album-web-test-source}"
SERVER_TARGET_DIR="${SERVER_TARGET_DIR:-/www/wwwroot/pic-test.jfyuntu.com/current}"
PUBLIC_URL="${PUBLIC_URL:-https://pic-test.jfyuntu.com/}"
...
log "Deploying album-web test from $REMOTE_NAME/$BRANCH@$LOCAL_SHA"
...
if ! command -v node >/dev/null 2>&1; then
  fail "Node is not installed on server."
fi
...
log "Building album-web against test API"
cd "$SOURCE_DIR/album-web"
pnpm install --frozen-lockfile
PUBLIC_JFYUNTU_BASE_PATH=/ PUBLIC_API_BASE=https://api-test.jfyuntu.com/api pnpm build

[[ -d dist ]] || fail "Build did not create album-web/dist."
...
log "album-web test deploy completed at $CURRENT_SHA"
REMOTE_SCRIPT
