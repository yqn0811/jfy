					...
					<view class="row-content">
						<view class="row" :style="{ 'height': rowHeight + 'rpx' }">
							<view class="row-name-wrap">
								<image class="icon" src="/static/icon/Frame 1171279070@2x.png"></image>
								<view class="text">{{ shadowRow.name }}({{ shadowRow.cont }})</view>
							</view>
							<view v-if="shadowRow.desc" class="desc">{{ shadowRow.desc }}</view>
						</view>
					</view>
					<!-- 拖拽图标 -->
						...
						<view class="modules">
							...
							<view class="row-content">
								<view class="row" @tap="triggerClick(row.HMDrag_sort, row)"
									...
									<view class="row-name-wrap">
										<image class="icon" src="/static/icon/Frame 1171279070@2x.png"></image>
										<view class="text">{{ row.name }}({{ row.cont }})</view>
									</view>
									<view v-if="row.desc" class="desc">{{ row.desc }}</view>
								</view>
							</view>
							<!-- 拖拽图标 -->
...
<script>
	...
	export default {
		...
		mounted() {
			...
			const res = this.$base.getSystemInfoCompat();
			// #ifdef MP-WEIXIN
