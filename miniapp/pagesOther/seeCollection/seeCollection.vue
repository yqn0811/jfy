    ...
    <view class="content" :style="{ paddingTop: totalHeight + 'px' }">
      ...
      <view class="search-box">
        <view class="search-input">
          ...
          <input
            ...
            :placeholder="placeholderFor('collectionSearch', '输入名称')"
            v-model="searchKeyword"
            @tap="focusField('collectionSearch')"
            @focus="focusField('collectionSearch')"
            @blur="blurField('collectionSearch')"
      ...
      >
        <view class="empty-box" v-if="visitList.length == 0 && !loading">
          <view class="tip">这里还没有收藏记录哦~</view>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      return this.$go("user/collect/records", data, "get", {
        show_err: true,
        loading: this.page === 1,
      })
        .then((res) => {
